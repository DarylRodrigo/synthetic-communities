...
import { usePopulationVotes, useRawData, useSimulationMetadata } from '@/lib/DataStore';
...
export default function VotesTab() {
    ...
    // Get available topics from metadata or debates
    const availableTopics = metadata?.topics?.map(topic => ({
        id: topic.id,
        title: topic.title
    })) || rawData?.debates?.map(debate => {
        const topic = metadata?.topics.find(t => t.id === debate.topic_id);
        return {
            id: debate.topic_id,
            title: topic?.title || debate.topic_id
        };
    }) || [];
    ...
    const getCandidateName = (candidateId: string) => {
        const candidate = metadata?.candidates.find(c => c.id === candidateId);
        return candidate?.name || candidateId;
    };

    const getVoterName = (voterId: string) => {
        const persona = metadata?.population.find(p => p.id === voterId);
        return persona?.demographics.name || voterId;
    };

    const getVoterDemographics = (voterId: string) => {
        const persona = metadata?.population.find(p => p.id === voterId);
        return persona?.demographics;
    };
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-3">
                    {filteredVotes.map((voter) => {
                        const voterName = getVoterName(voter.id);
                        const demographics = getVoterDemographics(voter.id);
                        ...
                        return (
                            <div
                                ...
                                title={`${voterName} - ${getCandidateName(voter.overall_vote)}`}
                            >
            ...
            {selectedVoter && (
                <div className="border-t border-gray-200 p-6 bg-gray-50">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h3 className="text-lg font-medium text-gray-900">{getVoterName(selectedVoter.id)}</h3>
                            {(() => {
                                const demographics = getVoterDemographics(selectedVoter.id);
                                return demographics ? (
                                    <p className="text-sm text-gray-600">
                                        {demographics.age} years old • {demographics.job} • {demographics.city}
                                    </p>
                                ) : (
                                    <p className="text-sm text-gray-600">Demographics not available</p>
                                );
                            })()}
                        </div>
                        <button
